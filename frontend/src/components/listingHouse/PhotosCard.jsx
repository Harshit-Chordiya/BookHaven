...
const PhotosCard = () => {
  ...
  const handleImageSelect = (event) => {
    /* This code block is handling the selection of an image file.
    It checks if the number of images
    already selected is equal to 3. 
    If it is, it displays an error message using the `toast.error()`
    function and returns, preventing further execution of the code. 
    If the number of images is less
  ...
  useEffect(() => {
    async function uploadImagetoCloudinary() {
      if (inputImage !== null && inputImage?.size / 500000 < 5) {
        ...
        imageFormData.append("upload_preset", "preset_name_azaz");
        imageFormData.append("cloud_name", "dd5nhhky2");

        // saving to cloudinary
        setIsImgUploading(true);
        try {
          await fetch(
            "https://api.cloudinary.com/v1_1/dd5nhhky2/image/upload",
            {
              method: "POST",
              body: imageFormData,
            }
